...
}_this.findElementDefaultActive();_this.handleDebugLine();_this.handleScrollListener();_this.handleOutput();_this.handleOnInitCallbacks();});_defineProperty(this,"shouldTerminate",function(){if(!_this.allowedPaths)return false;var currentPath=document.location.pathname.trim();var pathMatchesAllowedPaths=_this.allowedPaths.some(function(el){return currentPath.match(el.trim());});return!pathMatchesAllowedPaths;});_defineProperty(this,"findLinks",function(){_this.navLinks=_toConsumableArray(document.querySelectorAll(_this.navLinksSelector));});_defineProperty(this,"findRelatedSections",function(){var hashes=_this.navLinks.map(function(el){return el.hash;}).filter(function(el){return el;});_this.sections=_toConsumableArray(document.querySelectorAll(hashes.join(", ")));});_defineProperty(this,"handleScrollListener",function(){removeEventListener(scroll,_this.currentWindowScrollListener);_this.currentWindowScrollListener=window.addEventListener("scroll",function(){return _this.handleOutput();});});_defineProperty(this,"handleOutput",function(){var foundSection=_this.findCurrentSection();_this.updateCurrentSection(foundSection);if(_this.sectionChanged()||_this.newlyInitialized){_this.newlyInitialized=false;_this.clearClasses();_this.handleDefaultLinkActive();_this.handleHash();if(!_this.noSectionActive())_this.handleOnChangeCallbacks();if(_this.noSectionActive())return;_this.handleSectionClasses();_this.addActiveClassesOnNavigationLinks();_this.handleOnChangeCallbacks();}});_defineProperty(this,"findFirstSectionAbove",function(){return _this.sections.filter(function(section){return _this.isBelowSectionOffsetTop(section);}).at(-1);});_defineProperty(this,"findCurrentSection",function(){if(_this.exactMatch)return _this.sections.filter(function(section){return _this.isExactlyInsideSection(section);}).at(-1);else return _this.findFirstSectionAbove();});_defineProperty(this,"updateCurrentSection",function(newSection){_this.previousSection=_this.currentSection;_this.currentSection=newSection!==null&&newSection!==void 0?newSection:_this.handleDefaultLinkActive();});_defineProperty(this,"clearClasses",function(){_this.navLinks.forEach(function(link){_this.removeActiveClass(link);});_this.sections.forEach(function(el){el.classList.remove(_this.sectionActiveClass);});});_defineProperty(this,"handleDefaultLinkActive",function(){if(_this.noSectionActive()&&_this.defaultLinkActive){var _this$elementDefaultA;_this.addActiveClass((_this$elementDefaultA=_this.elementDefaultActive)!==null&&_this$elementDefaultA!==void 0?_this$elementDefaultA:_this.navLinks[0]);}});_defineProperty(this,"handleHash",function(){if(!_this.updateHash)return;if(_this.exactMatch&&_this.noSectionActive()&&_this.saveHashBetweenSections&&_this.passedBeginningOfFirstArticle()){var lastSectionAbove=_this.findFirstSectionAbove();history.pushState({},"",lastSectionAbove?"#"+lastSectionAbove.id:" ");}else{history.pushState({},"",_this.currentSection?"#"+_this.currentSection.id:" ");}});_defineProperty(this,"handleSectionClasses",function(){if(_this.setClassesOnSections){_this.currentSection.classList.add(_this.sectionActiveClass);}});_defineProperty(this,"addActiveClassesOnNavigationLinks",function(){var currentlyActiveLinks=_this.findLinksWithHashEqualTocurrentSectionId(_this.navLinks,_this.currentSection.id);currentlyActiveLinks.forEach(function(activeLink){return _this.addActiveClass(activeLink);});});_defineProperty(this,"findLinksWithHashEqualTocurrentSectionId",function(links,hash){return links.filter(function(el){return el.hash.includes(hash);});});_defineProperty(this,"addActiveClass",function(el){if(_this.updateATagClass){el.classList.add(_this.anchorActiveClass);}_this.parentsObtainingActiveClass.forEach(function(parentSelector){el.closest(parentSelector).classList.add(_this.anchorActiveClass);});});_defineProperty(this,"removeActiveClass",function(el){if(_this.updateATagClass){el.classList.remove(_this.anchorActiveClass);}_this.parentsObtainingActiveClass.forEach(function(parentSelector){el.closest(parentSelector).classList.remove(_this.anchorActiveClass);});});_defineProperty(this,"findElementDefaultActive",function(){try{_this.elementDefaultActive=document.querySelector(_this.defaultActiveElement);}catch(_unused){_this.elementDefaultActive=undefined;console.debug("Default active element not found.");}});_defineProperty(this,"handleOnInitCallbacks",function(){_this.onChange.forEach(function(callback){callback(_this);});});_defineProperty(this,"handleOnChangeCallbacks",function(){_this.onChange.forEach(function(callback){callback(_this);});});_defineProperty(this,"sectionChanged",function(){return _this.currentSection!=_this.previousSection;});_defineProperty(this,"noSectionActive",function(){return _this.currentSection==undefined;});_defineProperty(this,"currentOffset",function(){return window.scrollY+window.innerHeight/(100/_this.changeOffset);});_defineProperty(this,"passedBeginningOfFirstArticle",function(){return _this.currentOffset()>_this.sections.at(0).getBoundingClientRect().top+document.documentElement.scrollTop;});_defineProperty(this,"isExactlyInsideSection",function(section){return _this.isBelowSectionOffsetTop(section)&&_this.isAboveSectionOffsetBottom(section);});_defineProperty(this,"isAboveSectionOffsetTop",function(section){if(!section){return false;}return _this.currentOffset()<section.getBoundingClientRect().top+document.documentElement.scrollTop;});_defineProperty(this,"isAboveSectionOffsetBottom",function(section){if(!section){return false;}return _this.currentOffset()<section.getBoundingClientRect().top+document.documentElement.scrollTop+section.offsetHeight;});_defineProperty(this,"isBelowSectionOffsetTop",function(section){if(!section){return false;}return _this.currentOffset()>section.getBoundingClientRect().top+document.documentElement.scrollTop;});_defineProperty(this,"isBelowSectionOffsetBottom",function(section){if(!section){return false;}return _this.currentOffset()>section.getBoundingClientRect().top+document.documentElement.scrollTop+section.offsetHeight;});_defineProperty(this,"handleDebugLine",function(){if(_this.debugLine&&!_this.showDebugLine)_this.deleteDebugLine();if(_this.showDebugLine)_this.debugLine?_this.updateDebugLineOffset():_this.createDebugLine();});_defineProperty(this,"deleteDebugLine",function(){_this.debugLine.remove();_this.debugLine=undefined;});_defineProperty(this,"createDebugLine",function(){var debugLine=document.createElement("div");debugLine.setAttribute("style","position: fixed;width: 100%;background: red;height: 2px;transition: .6s; top:50%;");_this.debugLine=debugLine;_this.updateDebugLineOffset();document.body.appendChild(debugLine);});_defineProperty(this,"updateDebugLineOffset",function(){_this.debugLine.style.top="calc(".concat(_this.changeOffset,"% - 1px)");});this.navLinksSelector=navLinksSelector;this.defaultLinkActive=defaultLinkActive;this.updateATagClass=updateATagClass;this.changeOffset=changeOffset;this.parentsObtainingActiveClass=parentsObtainingActiveClass;this.setClassesOnSections=setClassesOnSections;this.exactMatch=exactMatch;this.allowedPaths=allowedPaths;this.anchorActiveClass=anchorActiveClass;this.sectionActiveClass=sectionActiveClass;this.defaultActiveElement=defaultActiveElement;this.updateHash=updateHash;this.saveHashBetweenSections=saveHashBetweenSections;this.onInit=onInit;this.onChange=onChange;this.showDebugLine=_debugLine;this.debugLine=undefined;this.sections=[];this.initialize();});
