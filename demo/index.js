...
sliderOptions.forEach((el) =>
    el.addEventListener('change', () => {
        onePageNav.set(el.name, el.checked ? true : false);
        if (el.name == 'setClassesOnSections')
            el.checked ? body.classList.add('sections-on') : body.classList.remove('sections-on');
    })
);
...
offset.addEventListener('change', () => {
    onePageNav.set(offset.name, offset.value);
});
...
defaultActiveAnchor.addEventListener('input', () => {
    onePageNav.set(defaultActiveAnchor.name, defaultActiveAnchor.value);
});
